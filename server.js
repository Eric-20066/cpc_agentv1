...
/* ===========================================================
   DETECTOR E CONVERSÃO CPC (SEU PARSER)
   =========================================================== */
...
async function callGroq(systemPrompt, userMessage) {
  ...
  return resp?.data?.choices?.[0]?.message?.content || "(Resposta vazia da API)";
}
...
app.post("/api/chat", async (req, res) => {
  ...
  // Converter CPC → PT (mantido)
  if (detectSymbolicFormula(raw)) {
    try { return res.json({ reply: toPortuguese(parseFormula(tokenize(raw))) }); }
    catch (e) { return res.json({ reply: "Erro na fórmula: " + e.message }); }
  }

  // → Agora tudo que for português vai para a IA traduzir
  try {
    return res.json({
      reply: await callGroq(
        "Você é um especialista em lógica proposicional. Converta textos em português para fórmulas lógicas CPC (¬ ∧ ∨ → ↔). Apenas a fórmula como saída.",
        raw
      )
    });
  } catch (e) {
